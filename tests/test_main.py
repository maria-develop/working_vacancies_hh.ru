...
def test_user_interaction_search(mock_input, MockJSONSaver, MockHeadHunterAPI):
    ...
    mock_hh_api.get_vacancies.return_value = [
        {'id': '1', 'name': 'Python Developer', 'area': {'name': 'Москва'}, 'url': 'http://example.com', 'salary': {'from': 1000}, 'description': 'Test description', 'snippet': {'requirement': 'Python', 'responsibility': 'Developing applications'}}
    ]
...
def test_user_interaction_filter_keyword(mock_input, MockJSONSaver):
    mock_json_saver = MockJSONSaver.return_value
    mock_json_saver.load_vacancies.return_value = [
        MagicMock(name='vacancy', description='Test description', snippet={'requirement': 'Python', 'responsibility': 'Developing applications'}),
        MagicMock(name='vacancy', description='Another description', snippet={'requirement': 'Java', 'responsibility': 'Developing Java applications'})
    ]
